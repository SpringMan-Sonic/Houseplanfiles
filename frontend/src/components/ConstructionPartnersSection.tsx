  ...
  >
    ...
    <div className="px-5 pb-5 flex flex-col flex-grow relative">
      ...
      <div className="pt-5 mt-auto flex flex-col gap-2">
        {type === "Premium" && (
          <Button 
            onClick={() => router.push(`/contractors/${partner._id}`)}
            variant="outline" 
                ...
                {filteredPartners.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
                    {filteredPartners.slice(0, 8).map((partner, index) => (
                      <PartnerCard key={partner._id} partner={partner} onContact={handleContactClick} index={index} navigate={(p) => router.push(p)} />
                    ))}
                  </div>
                )}
