...
import { LayoutDashboard, Box, PlusCircle, UserCircle, LogOut, X, ChevronDown, Briefcase, Building } from "lucide-react";
...
const SellerSidebar = ({ isOpen, setIsOpen }) => {
  ...
  const NavItem = ({ link, exact = false }) => {
    ...
    return (
      <Link href={link.path} onClick={() => setIsOpen(false)} className={`${baseLinkClasses} my-1 ${isActive ? activeClasses : inactiveClasses}`}>
        <link.icon className="h-5 w-5 mr-3" /><span>{link.name}</span>
      </Link>
    );
  };
  ...
  const renderSubMenu = (title, icon, menuKey, links) => (
    <div className="mt-2">
      <button onClick={() => toggleMenu(menuKey)} className={`${baseLinkClasses} ${inactiveClasses} justify-between`}>
        <div className="flex items-center">{React.createElement(icon, { className: "h-5 w-5 mr-3" })}<span>{title}</span></div>
        <ChevronDown className={`h-5 w-5 transition-transform ${openMenus[menuKey] ? "rotate-180" : ""}`} />
      </button>
      <AnimatePresence>
        {openMenus[menuKey] && (
          <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="overflow-hidden pl-4 border-l-2 border-slate-700 ml-3">
  ...
  return (
    <>
      <AnimatePresence>
        {isOpen && (<motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setIsOpen(false)} className="fixed inset-0 bg-black/60 z-30 lg:hidden" />)}
      </AnimatePresence>
      <aside className={`fixed top-0 left-0 h-full w-64 bg-slate-800 text-white p-4 shadow-2xl flex flex-col z-40 transition-transform duration-300 ease-in-out lg:translate-x-0 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
        <div className="flex items-center justify-between mb-6 border-b border-slate-700 pb-4">
          <Link href="/seller"><h1 className="text-2xl font-bold text-white">Seller Panel</h1></Link>
          <button onClick={() => setIsOpen(false)} className="lg:hidden text-gray-400 hover:text-white" aria-label="Close sidebar"><X className="h-6 w-6" /></button>
        </div>
        <nav className="flex-grow overflow-y-auto pr-2">
          {mainLinks.map((link) => <NavItem key={link.name} link={link} exact={link.path === "/seller"} />)}
            ...
            <div className="p-3 mb-2 rounded-md bg-slate-700/50">
              <div className="flex items-center">
                ...
                <div className="flex flex-col">
                  <span className="font-semibold text-white text-sm leading-tight">{userInfo.name || userInfo.businessName}</span>
